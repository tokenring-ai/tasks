...
describe("runTasks Tool", () => {
  ...
  beforeEach(() => {
    ...
    spyOn(taskService, "executeTasks").mockResolvedValue(["executed!"]);
  });
  ...
  describe("execute", () => {
    ...
    it("should execute tasks successfully with user approval", async () => {
      spyOn(agent, "askForApproval").mockResolvedValue(true);
      spyOn(taskService, "executeTasks").mockResolvedValue(["✓ Process Data: Completed", "✓ Send Email: Completed"]);
      spyOn(taskService, "addTask").mockImplementation((task, _agent) => `task-id-${task.name}`);
      ...
      expect(result).toEqual({
        ...
        attachments: [
          {
            sendToLLM: true,
            name: "Task Result (Process Data)",
            mimeType: "text/markdown",
            encoding: "text",
            body: "✓ Process Data: Completed",
          },
          {
            sendToLLM: true,
            name: "Task Result (Send Email)",
            mimeType: "text/markdown",
            encoding: "text",
            body: "✓ Send Email: Completed",
          },
        ],
      });
    });
    ...
    it("should handle user rejection", async () => {
      spyOn(agent, "askForApproval").mockResolvedValue(false);
      spyOn(agent, "askForText").mockResolvedValue("The plan is not clear enough");
      agent.config.headless = false;

      await expect(runTasks.execute({ tasks: validTasks }, agent)).rejects.toThrow("Task plan rejected. Reason: The plan is not clear enough");
    });
    ...
    it("should handle multiple tasks with different agents", async () => {
      ...
      expect(taskService.executeTasks).toHaveBeenCalledWith(expect.arrayContaining([expect.any(String), expect.any(String), expect.any(String)]), agent);
    });
  });
  ...
  describe("User Interaction", () => {
    ...
    it("should handle human input for rejection reason", async () => {
      ...
      ).rejects.toThrow("Task plan rejected. Reason: The plan is not clear enough");
